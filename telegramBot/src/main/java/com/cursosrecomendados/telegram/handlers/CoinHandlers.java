...
public class CoinHandlers extends TelegramLongPollingBot {
	...
	private synchronized void handleIncomingMessage(Message message) throws TelegramApiException {
		...
		case MAINMENU:
			...
			break;
		case COININFO:
			sendMessageRequest = messageOnCoin(message, language);
			break;
		case SETTINGS:
	...
	private static SendMessage messageOnMainMenu(Message message, String language) {
        SendMessage sendMessageRequest;
        if (message.hasText()) {
        	 if (message.getText().equals(getSettingsCommand(language))) {
	...
	private static SendMessage onCurrencyChosen(Integer userId, Long chatId, Integer messageId, String units,
			String language) {
